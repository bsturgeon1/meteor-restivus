        ...
        return {
            get: {
                action: async function () {
                    ...
                    const entity = await collection.find(selector).fetchAsync();
                    if (!_.isEmpty(entity)) {
                        return {
                            status: 'success',
                            data: entity
        ...
        return {
            put: {
                action: async function () {
                    ...
                    if (entityIsUpdated) {
                        const entity = await collection.findOneAsync(this.urlParams.id);
                        return {
                            status: 'success',
                            data: entity
                        };
        ...
        return {
            patch: {
                action: async function () {
                    ...
                    if (entityIsUpdated) {
                        const entity = await collection.findOneAsync(selector);
                        return {
                            status: 'success',
                            data: entity
                        };
        ...
        return {
            post: {
                action: async function () {
                    ...
                    const entity = await collection.findOneAsync(entityId);
                    if (!_.isEmpty(entity)) {
                        return {
                            statusCode: 201,
                            body: {
    ...
    getAll: function (collection) {
        return {
            get: {
                action: async function () {
                    const selector = getQueryParameters(this.queryParams);
                    const entities = await collection.find(selector).fetchAsync();
                    if (entities) {
                        return {
                            status: 'success',
                            data: entities
        ...
        return {
            put: {
                action: async function () {
                    ...
                    if (entityIsUpdated) {
                        const entity = collection.findOneAsync(this.urlParams.id, {
                            fields: {
                                profile: 1
                            }
                        });
